...
public class SemEvalTaskA {
	...
	public static void main(String[] args) throws Exception {
		...
		// tweetsPath "/home/yasen/projects/SU/text-mining-2014/data/no_download/train/cleansed/A"
		String tweetsPath = args[2];
		
		SerialDataStore sds = startGate(gatePath, dataStorePath);

		Corpus c = Factory.newCorpus("semEvalA");
		...
		for(File tweets : tweetsDir.listFiles()) {
			try {
				...
				List<Anno> twAnns = new ArrayList<Anno>();
				...
				while ((line = reader.readLine()) != null) {
					...
					String id1 = parts[0];
					String id2 = parts[1];
					int startTok = Integer.parseInt(parts[2]); // zero-based
					int endTok = Integer.parseInt(parts[3]);
					...
					// get exact start and end position of the phrase in text
					String[] toks = text.split(" ");
					int startPos = 0;
					int sp;
					for(sp = 0; sp < startTok; ++sp) {
						startPos += toks[sp].length() + 1; // " ".length();
					}
					int endPos = startPos;
					for(int ep = sp; ep <= endTok; ++ep) {
						endPos += toks[ep].length() + 1;
					}
					// trailing space
					endPos--;
					...
					count++;
					if(count % 500 == 0) {
						addDocument(docAnns, twAnns, docText.toString(), sds, corpus, tweets.getName() + i);
						++i;
						docText = new StringBuilder();
						docAnns = new ArrayList<Anno>();
						twAnns = new ArrayList<Anno>();
					}
				}
				addDocument(docAnns, twAnns, docText.toString(), sds, corpus, tweets.getName() + i);
	...
	private static void addDocument(List<Anno> docAnnots, List<Anno> twAnns, String docText, SerialDataStore ds, Corpus cor, String docName)
			throws Exception {
